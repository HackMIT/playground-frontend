    ...
    for (var i = 0; i < messages.length; i++) {
      ...
      if (data.type === "init") {
        ...
        for (let [id, hallway] of Object.entries(data.room.hallways)) {
          ...
          document
            .getElementById("game")
            .appendChild(this.hallways[id].element);
        }
        ...
        if (this.room.sponsor) {
          ...
          document.getElementById("sponsor-name").innerHTML =
            "<span>" + this.room.slug + "</span>" + this.room.slug;
        ...
        document.getElementById("game").style.backgroundImage =
          "url('" +
          BACKGROUND_IMAGE_URL.replace("%SLUG%", this.room.slug) +
          "')";
        ...
        this.scene.moveCharacter(data.id, data.x, data.y, () => {
          ...
          for (let [id, hallway] of Object.entries(this.hallways)) {
            let distance = Math.sqrt(
              Math.pow(hallway.data.x - data.x, 2) +
                Math.pow(hallway.data.y - data.y, 2)
            );
